import { route, type RouteContext } from "@/lib/api/handler";
import { ok } from "@/lib/api/response";
import { requirePermission } from "@/lib/auth/authorize";
import { PERMISSIONS } from "@/lib/auth/permissions";
import { getPlayerInsights } from "@/lib/services/player.service";

// Form, streak, head-to-head rivalries + achievement badges for a player.
export const GET = route<{ id: string }>(async (_req, { params }: RouteContext<{ id: string }>) => {
  const actor = await requirePermission(PERMISSIONS.PLAYER_VIEW);
  const { id } = await params;
  return ok(await getPlayerInsights(actor, id));
});
